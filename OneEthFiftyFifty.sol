...
contract OneEthFiftyFifty is usingProvable
{
    ...
    function deposit() payable public {
        require(msg.value == 1000000000000000000); // only accept 1 eth as payment
        require(activeContract = true); // check to make sure the contract hasn't already been used
        require(address(this).balance <= 2000000000000000000); // stop people from sending multiple deposits in the same block
        users.push(msg.sender); // add the address that sent it to our users array
       
        if(users.length == 2 && address(this).balance == 2000000000000000000){ // if two people have entered the bet
