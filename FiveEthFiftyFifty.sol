...
contract FiveEthFiftyFifty is usingProvable
{
    ...
    function deposit() payable public {
        require(msg.value == 5000000000000000000); // only accept 5 eth as payment
        require(activeContract = true); // check to make sure the contract hasn't already been used
        require(address(this).balance <= 10000000000000000000); // stop people from sending multiple deposits in the same block
        users.push(msg.sender); // add the address that sent it to our users array
       
        if(users.length == 2 && address(this).balance == 10000000000000000000){ // if two people have entered the bet
    ...
    function chooseWinner(address payable[] memory userArray, uint winningNumber) internal { // code to determine the winner
        ...
        if(winningNumber == 0){ // if the first user wins
            userArray[0].transfer(address(this).balance); // send the entire amount (10 eth)
            emit LogWinner(userArray[0]);
        }else if(winningNumber == 1){  // if the second user wins
            userArray[1].transfer(address(this).balance); // send the entire amount (10 eth)
